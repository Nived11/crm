...
import {
  ...
  MoreVertical, Inbox, AlertCircle, FileText, 
  Building, Tag,
  Building2Icon
} from "lucide-react";
...
const ClientManager = () => {
  ...
  const [clientForm, setClientForm] = useState({
    owner_name: "", company_name: "", phone_number: "", location: "", category: "", note: ""
  });
  ...
  const openModal = (type: 'client' | 'category' | 'status', data: any = null) => {
    if (type === 'client') {
      ...
      if (data) {
        ...
        setClientForm({ owner_name: "", company_name: "", phone_number: "", location: "", category: "", note: "" });
      }
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    const finalData = {
      ...clientForm,
      owner_name: clientForm.owner_name?.trim() || "N/A"
    };
    ...
    <div className="bg-black min-h-screen pb-10">
                      ...
                      <tr key={client.id} className="hover:bg-zinc-900/20 transition-colors group">
                        ...
                        <td className="p-6">
                          <a
                            ...
                            className="flex items-center gap-2 text-sm text-zinc-400 font-medium hover:text-brand transition-colors"
                          >
                            <Phone size={12} className="text-zinc-700" />
                            <span>+91 {client.phone_number}</span>
                          </a>
                        </td>
                        ...
                        <td className="p-6">
                          <button
                            ...
                            className={`cursor-pointer flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${client.has_called
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex flex-col items-end gap-2">
                            <span className="text-[9px] text-zinc-600 font-bold uppercase tracking-tighter mb-1">
                              {formatDateTime(client.created_at)}
                            </span>

                            <div className="flex items-center justify-end gap-2">
                              {client.note && client.note.trim() !== "" && (
                                <button onClick={() => handleViewNote(client.note)} className="p-2 bg-zinc-900 rounded-lg text-amber-500 hover:text-white transition-all border border-zinc-800" title="View Note">
                                  <FileText size={14} />
                                </button>
                              )}
                              <button onClick={() => openModal('client', client)} className="p-2 bg-zinc-900 rounded-lg text-zinc-500 hover:text-white transition-all border border-zinc-800"><Edit3 size={14} /></button>
                              <button onClick={() => { setSelectedClient(client); setIsDeleteModalOpen(true); }} className="p-2 bg-zinc-900 rounded-lg text-zinc-500 hover:text-red-500 transition-all border border-zinc-800"><Trash2 size={14} /></button>
                            </div>
                          </div>
                        </td>
                      </tr>
                ...
                {clients.map((client: any) => (
                  <div key={client.id} className="p-5 flex flex-col gap-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <h4 className="text-[12px] text-zinc-200 font-semibold uppercase tracking-wider">
                              {client.company_name}
                            </h4>
                          </div>
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-1.5 text-[9px] text-zinc-600 font-medium">
                              ...
                              <span className="text-zinc-400 text-[10px]">{client.category_name}</span>
                            </div>
                            {/* Owner Name with Icon */}
                            <div className="flex items-center gap-1.5 text-[9px] text-zinc-600 font-medium">
                              <MapPin size={10} className="text-zinc-500" />
                              <span className="text-zinc-400 text-[10px]">{client.location}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <User size={10} className="text-zinc-600" />
                              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                                {client.owner_name ? client.owner_name : "N/A"}
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* ... Rest of your menu/buttons code ... */}
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[8px] text-zinc-600 font-medium uppercase tracking-tighter">
                          {formatDateTime(client.created_at)}
                        </span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {client.note && client.note.trim() !== "" && (
                            <button
                              onClick={() => handleViewNote(client.note)}
                              className="p-2 bg-zinc-900 rounded-lg text-amber-500 hover:text-white transition-all"
                            >
                              <FileText size={18} />
                            </button>
                          )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="bg-zinc-900/40 p-1 rounded-xl border border-zinc-900 flex items-center justify-center">
                        <button
                          disabled={client.has_called}
                          onClick={() => openModal('status', client)}
                          className={`w-full h-full flex items-center justify-center gap-2 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${client.has_called
      ...
      {isNoteViewModalOpen && (
        <div
          ...
          className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in zoom-in-95 duration-200"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-[#0c0c0c] w-full max-w-xl rounded-2xl border border-zinc-800 p-4 shadow-2xl relative"
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-[10px] font-black text-white uppercase tracking-[0.2em] flex items-center gap-2">
                <FileText size={16} className="text-zinc-500" /> Note View
              </h2>
              <button onClick={closeAllModals} className="p-2 bg-zinc-900 rounded-full text-zinc-500 hover:text-white transition-all">
                <X size={16} />
              </button>
            </div>
            <div className="bg-zinc-900/30 border border-zinc-800 p-4 rounded-xl max-h-[80vh] overflow-y-auto">
              <p className="text-xs md:text-sm text-zinc-300 whitespace-pre-wrap leading-relaxed">{viewNoteText}</p>
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleAddClient} className="p-6 md:p-10 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input value={clientForm.owner_name} onChange={(e) => setClientForm({ ...clientForm, owner_name: e.target.value })} className="w-full bg-zinc-900/30 border border-zinc-800 p-4 pl-12 rounded-2xl text-sm text-white outline-none focus:border-brand/40" placeholder="Enter Owner Name" />
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest ml-1">Note</label>
                <textarea
                  value={clientForm.note}
                  onChange={(e) => setClientForm({ ...clientForm, note: e.target.value })}
                  className="w-full bg-zinc-900/30 border border-zinc-800 p-4 rounded-2xl text-sm text-white outline-none focus:border-brand/40"
